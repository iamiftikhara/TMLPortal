...
;(function ( $, window, document, undefined ) { 
  ...
  ComboTree.prototype.createSelectAllHTMLForMultiSelect = function () {
    return '<label class="selectAll"><input type="checkbox" name="file" id="' + this.comboTreeId + 'SelectAll' + '">[Select All]</label>';
  }

  ComboTree.prototype.createSourceSubItemsHTML = function (subItems, parentId, collapse=false,parentIdclass) {
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem ,mostChildParent) {
    var itemHtml = "",
    ...
    if (this.options.isMultiple)
      itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? '<input type="checkbox" name="file" id="' + sourceItem.id + '"  value="' + sourceItem.title + '" class="' + sourceItem.class + ' mostNestedChild checkbox"  data-custom-placeholder="' + sourceItem.class + '"/>' : '') + sourceItem.title + '</span>';
  ...
  if ($(ctItem).data("selectable") == true) {
    ...
    if (check) {
      ...
      if (index) {
        ...
        this.logSelectedChildrenCount($(ctItem).parent('li'),ctItem);
        // Remove parent data corresponding to the removed child
        var parent = $(ctItem).closest('ul').closest('li.ComboTreeItemParent');
        if (parent.length > 0) {
          var parentId = parent.find('input[type="checkbox"]').attr("id");
          var parentDataIndex = this._selectedItems.findIndex(item => item.id === parentId);
          if (parentDataIndex !== -1) {
            this._selectedItems.splice(parentDataIndex, 1);
          }
        }
      } else {
        this._selectedItems.push(this._selectedItem);
        $(ctItem).find("input").prop('checked', true);
        // Log the total number of children
        // console.log("$(ctItem)",$(ctItem))
        var totalChildren = $(ctItem).closest('ul').find('input[type="checkbox"]').length;
        var checkedLength = $(ctItem).closest('ul').find('input[type="checkbox"]:checked').length;
        // console.log("checkedLength",checkedLength)
        // console.log("totalChildren",totalChildren)
        // console.log("Checked length",$(ctItem).closest('li').closest('ul').closest('li').closest('ul').find('input[type="checkbox"]:checked').length)
        // console.log("Total length",$(ctItem).closest('li').closest('ul').closest('li').closest('ul').find('input[type="checkbox"]').length)
        // Check if all children are selected, then select the parent
        if (checkedLength == totalChildren) {
          // Recursively check ancestors
          this.checkAncestors($(ctItem).parent('li'));
          // Add parent's data to the selection item list
          var parent = $(ctItem).closest('ul').closest('li.ComboTreeItemParent');
          console.log("parent",parent)
          if (parent.length > 0) {
            var parentId = parent.find('input[type="checkbox"]').attr("id");
            var parentValue = parent.find('input[type="checkbox"]').val();
            var parentData = {
              id: parentId,
              title: parentValue
            };
            // Check if parent data is not already in the list before adding
            if (!this.isItemInArray(parentData, this._selectedItems)) {
              this._selectedItems.push(parentData);
            }
          }
        }
      }
...
// Log selected children count function
// ComboTree.prototype.logSelectedChildrenCount = function(parentItem,ctItem) {
//   console.log("parentItem",$(parentItem).parent('ul').closest('li').closest('ul').siblings('span.comboTreeItemTitle').children('input[type="checkbox"]').prop('checked', false))
//   $(parentItem).parent('ul').closest('li').closest('ul').siblings('span.comboTreeItemTitle').children('input[type="checkbox"]').prop('checked', false)
//   $(parentItem).parent('ul').closest('li').closest('ul').closest('li').closest('ul').siblings('span.comboTreeItemTitle').children('input[type="checkbox"]').prop('checked', false)
//   $(parentItem).parent('ul').siblings('span.comboTreeItemTitle').children('input[type="checkbox"]').prop('checked', false);
// };

ComboTree.prototype.logSelectedChildrenCount = function(parentItem, ctItem) {
  // Uncheck parent checkboxes based on hierarchy
  var $parentUl = $(parentItem).parent('ul');
  
  // Condition 1
  $parentUl.closest('li').closest('ul').siblings('span.comboTreeItemTitle').children('input[type="checkbox"]').prop('checked', false);
  
  // Condition 2
  if ($parentUl.closest('li').closest('ul').closest('li').length > 0) {
    $parentUl.closest('li').closest('ul').closest('li').closest('ul').siblings('span.comboTreeItemTitle').children('input[type="checkbox"]').prop('checked', false);
  }

  // Condition 3
  $parentUl.siblings('span.comboTreeItemTitle').children('input[type="checkbox"]').prop('checked', false);
};

// Recursive function to check ancestors
// ComboTree.prototype.checkAncestors = function(parentItem) {
//   if (this.options.cascadeSelect && $(parentItem).parent('ul').length > 0) {
//     var allSiblingsSelected = $(parentItem).siblings('li').length === $(parentItem).siblings('li').filter(':has(input[type="checkbox"]:checked)').length;
//     $(parentItem).parent('ul').siblings('span.comboTreeItemTitle').children('input[type="checkbox"]').prop('checked', allSiblingsSelected);
//     this.checkAncestors($(parentItem).parent('ul').parent('li'));
//   }
// };
...
ComboTree.prototype.checkAncestors = function(parentItem) {
  ...
  if (this.options.cascadeSelect && $(parentItem).parent('ul').length > 0) {
    // console.log("8")
    var allChildrenSelected = $(parentItem).parent('ul').find('input[type="checkbox"]').length === $(parentItem).parent('ul').find('input[type="checkbox"]:checked').length;
    $(parentItem).parent('ul').siblings('span.comboTreeItemTitle').children('input[type="checkbox"]').prop('checked', allChildrenSelected);

    // Recursively check ancestors for cascading selection
    this.checkAncestors($(parentItem).parent('ul').parent('li'));

    // Select immediate children, including combo tree parents, and pass the selected parent's ID
    // var selectedParentId = $(parentItem).find('input[type="checkbox"]').attr('id');
    // this.selectImmediateChildren($(parentItem).parent('ul'), selectedParentId);
  }
};
// ComboTree.prototype.selectImmediateChildren = function(ulElement, selectedParentId) {
//   var childrenCheckboxes = ulElement.find('input[type="checkbox"]');
//   // Check only the immediate children that are present in the selected parent
//   childrenCheckboxes.each(function() {
//     var childId = $(this).attr('id');
//     if (childId.startsWith(selectedParentId)) {
...
// ComboTree.prototype.selectImmediateChildren = function(ulElement, selectedParentId) {
//   var childrenCheckboxes = ulElement.find('input[type="checkbox"]');
//   // var topLevelParent = ulElement.closest('li').closest('ul').closest('li').closest('ul').siblings('span.comboTreeItemTitle');

//   // Get the ID of the Phishing Simulator parent dynamically
//   // var phishingSimulatorParentId = ulElement.closest('.ComboTreeItemParent.PhishingSimulator').attr('id');
//   // var phishingSimulatorCheckboxLength = $('#' + phishingSimulatorParentId).find('input[type="checkbox"]').length;
...
//       // Recursively select descendants if the child is a Combo Tree parent
//       if ($(this).parent('span.comboTreeItemTitle').hasClass('comboTreeItemParent')) {
//         var childUl = $(this).siblings('ul');
//         if (childUl.length > 0) {
//           this.selectImmediateChildren(childUl, childId);
//         }
//       }
//     }
//   });
  ...
  // var immediateChildrenChecked = childrenCheckboxes.filter(':checked').length === childrenCheckboxes.length;

  // if (immediateChildrenChecked && phishingSimulatorCheckboxLength === childrenCheckboxes.length) {
  //   // Check the top-level parent
  //   topLevelParent.find('input[type="checkbox"]').prop('checked', true);
  // } else {
  //   // Uncheck the top-level parent if not all immediate children are checked
  //   topLevelParent.find('input[type="checkbox"]').prop('checked', false);
  // }

  // Uncheck other top-level parents before checking the current one
  // $('.ComboTreeItemParent').not(topLevelParent.closest('.ComboTreeItemParent')).find('input[type="checkbox"]').prop('checked', false);
// };
...
// ComboTree.prototype.selectImmediateChildren = function(ulElement, selectedParentId) {
//   var childrenCheckboxes = ulElement.find('input[type="checkbox"]');
//   var topLevelParent = ulElement.closest('li').closest('ul').closest('li.ComboTreeItemParent');
// console.log("topLevelParent",topLevelParent)
//   // Check only the immediate children that are present in the selected parent
//   childrenCheckboxes.each(function() {
//     var childId = $(this).attr('id');
//     if (childId.startsWith(selectedParentId)) {
...
//   // Check if all immediate children are checked
//   // console.log("checked legth",childrenCheckboxes.filter(':checked').length)
//   // console.log("total length legth",childrenCheckboxes.length)
//    console.log("Checked length",$(childrenCheckboxes).closest('li').closest('ul').closest('li').closest('ul').find('input[type="checkbox"]:checked').length)
//         console.log("Total length",$(childrenCheckboxes).closest('li').closest('ul').closest('li').closest('ul').find('input[type="checkbox"]').length)
//   var immediateChildrenChecked = $(childrenCheckboxes).closest('li').closest('ul').closest('li').closest('ul').find('input[type="checkbox"]:checked').length === $(childrenCheckboxes).closest('li').closest('ul').closest('li').closest('ul').find('input[type="checkbox"]').length;

//   if (immediateChildrenChecked) {
//     // Check the top-level parent
//     topLevelParent.find('input[type="checkbox"]').prop('checked', true);
//   }
//   // } else {
